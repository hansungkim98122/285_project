...
class PPO(nn.Module):
    def __init__(
        ...
        self.target_actor = make_actor(observation_shape, action_dim)
    ...
    def q_backup_strategy(self, next_qs: torch.Tensor) -> torch.Tensor:
        ...
        if self.target_critic_backup_type == "doubleq":
            ...
            next_qs = torch.minimum(next_qs[0,:],next_qs[1,:]) #Assuming two critic networks
        else:
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            target_values: torch.Tensor = reward[None] + self.discount*next_qs*(1-1.0*done[None])
    ...
    def actor_loss(self, obs: torch.Tensor,acs: torch.Tensor):
        batch_size = obs.shape[0]

        # TODO(student): Generate an action distribution
        action_distribution: torch.distributions.Distribution = self.actor(obs)
        old_action_distribution: torch.distributions.Distribution = self.target_actor(obs)
        
        cur_acs = action_distribution.rsample()
        with torch.no_grad():
        ...
        # TODO(student)
        log_probs = action_distribution.log_prob(cur_acs)
        old_log_probs = old_action_distribution.log_prob(acs)

        assert log_probs.shape == old_log_probs.shape

        r = log_probs - old_log_probs
        loss = -torch.min(r*advantage,torch.clip(r,1-self.eps_clip,1+self.eps_clip)*advantage).mean()

        return loss, torch.mean(self.entropy(action_distribution))

    def update_actor(self, obs: torch.Tensor, acs: torch.Tensor):
        """
    ...
    def update_target_actor(self):
        self.soft_update_target_actor(1.0)
    ...
    def soft_update_target_actor(self, tau):
        for target_param, param in zip(
    ...
    def update(
        ...
        actor_info = self.update_actor(obs=observations,acs=actions)

        if self.soft_target_update_rate:
            #Do soft updates
            self.soft_update_target_critic(self.soft_target_update_rate)
            self.soft_update_target_actor(self.soft_target_update_rate)
        else:
            #Do hard updates
